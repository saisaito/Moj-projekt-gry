...
public:
	string nazwa;
	int hp;					// Hp sta³e/maksymalne	
	int hp_p;				// Hp zmienne/w³aœciwe
	int mana;				// Mana sta³a/maksymalna 
	int mana_p;				// Mana zmienna/w³aœciwa
	int fatak;				// Obra¿enia od ataków fizycznych
	int matak;				// Obra¿enia od ataków magicznych
	int ap;					// Moc umiejêtnoœci - zmienna zwiêkszaj¹ca obra¿enia magiczne
	int ad;					// Atak fizyczny - zmienna zwiêkszaj¹ca obra¿enia fizyczne
	int luck;				// Zmienna która zwiêksza prawdopodobieñstwo
	int op;					// Zmienna w której bêdziê przechowywany wynik ataków
	int pan;				// Pancerz
	int r;					// Zmienna która bêdzie przetrzymywaæ wynik losowania 

	virtual bool is_death()			// sprawdza czy jest martwy
	{
		return hp_p <= 0;
	}
};
...
{
public:
	virtual void show_option()=0;
	virtual void select_option(char option)=0;
